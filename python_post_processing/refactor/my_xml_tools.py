...
class ReadSimXML:
    ...
    def parse_xml_content(self, content: str) -> ET.ElementTree:
        ...
        Args:
            content (str): XML content in string format.

        Returns:
            ET.ElementTree: Parsed XML content as an ElementTree object.

        Raises:
            ET.ParseError: If the content is not valid XML.
        """
        try:
    ...
    def extract_parameter_value(self, root: ET.Element, path: list) -> str:
        ...
        Args:
            ...
            path (list): The path to the parameter within the XML tree.
    ...
    def search(self, parameter_path: list) -> float:
        ...
        Args:
            parameter_path (list): The path to the parameter in the XML structure.

        Returns:
            float: The value of the parameter as a float, or None if not found.

        Raises:
            FileNotFoundError: If no XML files are found in the directory.
            ValueError: If the parameter value cannot be converted to a float.
        """
